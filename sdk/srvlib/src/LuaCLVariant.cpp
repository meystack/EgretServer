...
#include "../include/LuaCLVariant.h"
#include <new>

namespace CLVariantExport
{
	static const unsigned int RegistedTypeId = CCLVariant::hashstr("CCLVariant");
	static const unsigned int RegistedRefTypeId = CCLVariant::hashstr("CCLVariantRef");

	static CCLVariant* getDataPtr(lua_State *L, int index = 1)
	{
		if (lua_type(L, index) != LUA_TUSERDATA)
			return NULL;
		CCLVariant *result = NULL;
		/*
		...
		if (lua_getmetatable(L, index))
		{
			...
			if (lua_isnumber(L, -1))
			{
				...
				if (RegistedRefTypeId == tpid)
					result = *(CCLVariant**)lua_touserdata(L, (index > 0) ? index : index - 2);
	...
	static int newRef(lua_State *L, CCLVariant &var)
	{
	...
	static void setVarFromTable(lua_State *L, CCLVariant *pVar, int vidx = -1);
	static void setVarMember(lua_State *L, CCLVariant *pVar, const char* sName, int vidx = -1);
	static int newInst(lua_State *L)
	{
		CCLVariant *pVar = (CCLVariant*)lua_newuserdata(L, sizeof(CCLVariant));
		//为userdata设置类型表
	...
	static int ipair(lua_State *L)
	{
		CCLVariant *pVar = getDataPtr(L);
		if ((!pVar) || (pVar->type() != CCLVariant::vStruct))
		{
			lua_pushnil(L);
			return 1;
		}

		unsigned int idx = (unsigned int)lua_tonumber(L, 2) - 1;
		if (idx >= pVar->len())
		{
			lua_pushnil(L);
			return 1;
	...
	static void setVarFromTable(lua_State *L, CCLVariant *pVar, int vidx)
	{
		lua_pushnil(L);
		if (vidx < 0) vidx--;
	...
	static void setVarMember(lua_State *L, CCLVariant *pVar, const char* sName, int vidx)
	{
		...
		*/
		if (lua_getmetatable(L, 1))
		{
			...
			if (lua_isnumber(L, -1))
			{
				unsigned int tpid = (unsigned int)lua_tonumber(L, -1);
				if (RegistedRefTypeId == tpid)
					*(CCLVariant**)lua_touserdata(L, 1) = NULL;
				else if (RegistedTypeId == tpid)
					((CCLVariant*)lua_touserdata(L, 1))->~CCLVariant();
			}
	...
	static int mod(lua_State *L)
	{
		...
		{
		case LUA_TNUMBER:
			d = (__int64)lua_tonumber(L, 2);
			if (d != 0)
				lua_pushnumber(L, (lua_Number)(n % d));
			else lua_pushnumber(L, 0);
			break;
		case LUA_TSTRING:
			d = (__int64)lua_str2number(lua_tostring(L, 2), &e);
			if ((!e || !*e) && d != 0)
				lua_pushnumber(L, (lua_Number)(n % d));
			else lua_pushnumber(L, 0);
			break;
	...
	static void registTo(lua_State *L, const char *className)
	{
		...
		//ipair
		setMetaNameClosure(L, "ipair", (const void*)ipair);
		//__index
...
namespace LuaCLVariant
{
	...
	int returnValue(lua_State *L, CCLVariant &var)
	{
