...
inline bool fileExists(LPCTSTR sFilePath)
{
	...
	//如果是绝对路径
	if(sFilePath[0] =='/' || (sFilePath[0] =='.' &&  sFilePath[1] =='/') )
	{
		result = stat (sFilePath,&info);
	}
	else
	{
		char buff[512];
		strcpy(buff,"./");
		strcat(buff,sFilePath);
		result = stat (buff,&info);
		if(  result !=0)
		{
			//OutputMsg(rmTip, _T("False1 FilePath %s"),buff);
			return false;
		}
	...
	if(  result !=0)
	{
		//OutputMsg(rmTip, _T("False FilePath %s"),sFilePath);
		return false;
	}
	if( (info.st_mode & S_IFMT) == S_IFREG)
    {
		//OutputMsg(rmTip, _T("True FilePath %s"),sFilePath);
       	return true;
    }
	else
	{
...
VOID CCustomLuaPreProcessor::ProcessLine(LPCTSTR sLineText)
{
	...
	//取得预处理指令以及指令内容
	sLineText += 3;//调过--#
	while (nIdx <(INT_PTR) ArrayCount(sDirective)-1)
	{
...
LPCTSTR CCustomLuaPreProcessor::SaveFileDirAndParse(LPCTSTR sSourceText, LPCTSTR sFullFilePath)
{
	...
	//OutputMsg(rmTip,_T("add Dir:%s"), (LPTSTR)sDir.rawStr());
	...
	//strncpy( pLn->sFileName,sFullFilePath,ArrayCount(pLn->sFileName) - 1);
#else
	strncpy( pLn->sFileName,sFullFilePath,ArrayCount(pLn->sFileName) - 1);
#endif
...
#endif
	OutputMsg(rmError,_T("include file \"%s\" can not be found"), sIncludeFileName);
	return false;
}
...
bool CCustomLuaPreProcessor::LoadIncludeFile(LPCTSTR sIncludeFilePath, bool boEnableMultipleInclude)
{
	...
	DWORD dwLen =0;
#ifdef WIN32
	dwLen = GetFullPathName(sIncludeFilePath, dwBufLen - 1, sFilePath, NULL );
#else
	if( sIncludeFilePath[0] =='/') //是个盘符，说明是完整路径
	{
		strncpy(sFilePath,sIncludeFilePath,dwBufLen -1);
	}
	else
	{
		getcwd(sFilePath,dwBufLen - 1); //获取当前的路径
		//GetCurrentDirectory(dwBufLen - 1,sFilePath);
		int nStrLen = strlen(sFilePath);
		if(sFilePath[nStrLen -1] !='\\' && sFilePath[nStrLen -1] !='/'  &&sIncludeFilePath[0] != '\\' &&sIncludeFilePath[0] != '/'  )
		{
			strcat(sFilePath,"/");
		}
		strcat(sFilePath,sIncludeFilePath);
	}
	dwLen =strlen(sFilePath);
#endif

	//OutputMsg(rmTip,"OpenFile: %s",sFilePath);
	sFilePath[dwLen] = 0;
