...
#else
void CCustomLuaProfiler::StaticCallTimeHook(lua_State *L, lua_Debug *ar)
#endif
{
...
#else
			nLen = _sntprintf(sBuffer, ArrayCount(sBuffer), "%-40s %-8lld %-8lld %-8lld %-8lld\r\n",
				pRec->sName, (long long int)pRec->dwMinTime,(long long int) pRec->dwMaxTime, (long long int)pRec->dwCallCount, (long long int)pRec->dwTotalTime/pRec->dwCallCount);
#endif
	...
	CCustomLuaPreProcessor::CLineRangeList *pLRList)
{
	...
	*/
	...
	nLen = _sntprintf(sBuffer, ArrayCount(sBuffer), " %-8lld %-8lld %-8lld %-8lld ",
		(long long int)pRec->dwMinTime, (long long int)pRec->dwMaxTime, (long long int)pRec->dwCallCount, (long long int)pRec->dwTotalTime/pRec->dwCallCount);
#endif
	stm.write(sBuffer, nLen * sizeof(sBuffer[0]));
	...
	if (pLRList && pRec->nLineNo >= 0)
	{
		...
		if (sSrcFile)
		{
			...
			nLen = sprintf(sBuffer, ":%d", (int)nSrcNo);
			stm.write(sBuffer, nLen * sizeof(sBuffer[0]));
		}
	}
	stm.write("\r\n", 2);
