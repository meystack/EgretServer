...
;
;
;                     wylib 2006 - 2011 快速CRC运算函数
;
;
;    通过预先计算出CRC码表并使用汇编函数书写CRC计算函数，将码表通过内联到函数代
;  码中，使用CPU的一级指令缓存快速访问码表来提高函数效率。
;
;---------------------------------------------------------------------------------
...
CRC32Update   proc \
  ...
  MOV    EAX,DWORD PTR[ESP+24h] ;crc
  MOV    EDX,DWORD PTR[ESP+28h] ;data
  and    EDX,EDX
  JZ     @Exit
  MOV    ECX,DWORD PTR[ESP+2Ch] ;len
  and    ECX,ECX
  JLE    @Exit

  xor    EBX,EBX
  LEA    EDI,CS:[OFFSET @CRC32DT]
@Start:       
;  CRC32 := (CRC32 shr 8) xor Table[Buffer[i] xor (CRC32 and $000000ff)];
  ...
  JNZ    @Start
@Exit: 
  MOV    DWORD PTR [ESP+1Ch], EAX ;Save EAX to stack pointer for POPAD  
  POPAD
  RET 0Ch
@AlignCode_:
