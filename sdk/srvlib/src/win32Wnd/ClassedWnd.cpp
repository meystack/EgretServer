 ...
 *              
 * 将消息处理调用到类实例的内存补丁简析：            
 * 1)有用的结构有：ClassedWndProcInstance和CWInstanceBlock，两个结构中有成员说明；
 * 2)内存补丁的代码调用过程为：
...
typedef struct CWInstanceBlock
{
	...
	/**
	 * 指向下一个可用的分页窗口处理执行代码补丁块
	 */
	struct CWInstanceBlock*	Next;
...
HWND CClassedWnd::MyCreateWindowEx(DWORD dwExStyle, LPCTSTR lpClassName, LPCTSTR lpWindowName, DWORD dwStyle, int x, int y, 
