...
public:
	...
	// 跳过多少字节
	inline size_t Skip(size_t dwSize){
		size_t dwAvaliableSize = m_pDataEnd - m_pOffset;
		if ( dwSize > dwAvaliableSize )
			dwSize = dwAvaliableSize;
		if ( dwSize > 0 )
	...
	inline T readAtom()
	{
		T val = T(); // 基本类型、任意指针 初始化为 0
		INT_PTR dwAvaliableSize = m_pDataEnd - m_pOffset;
	...
	* 字符串的数据格式为：[2字节字符长度数据][字符串字节数据，字符串长度在65536以内][字符串终止字符0]
	...
	// 简化字符数组型的读取
	template<size_t N>
	inline size_t readString(OUT char (&str)[N])
	{
		return rawReadStringLen<char*, unsigned short>(str, N);
	}
	/* 读取UNICODE16短字符串数据
	* 字符串的数据格式为：[2字节字符长度数据][字符串字节数据，字符串长度在65536以内][字符串终止字符0]
	 * str			 字符串读取缓冲区指针
...
protected:
	...
	inline size_t rawReadStringLen(OUT TS str, size_t dwCharsToRead)
	{
		size_t dwAvaliableSize = m_pDataEnd - m_pOffset;
		size_t nReadLen = 0, nStrLen = 0;
		if ( m_pDataEnd >= m_pOffset + sizeof(TL) )
		{
			...
			if ( dwCharsToRead > 0 )
			{
				...
				if ( nStrLen > nReadLen )
				{
					...
					if(m_pOffset > m_pDataEnd) m_pOffset = m_pDataEnd;
				}

				//跳过终止字符0
				if ( m_pOffset + sizeof(*str) <= m_pDataEnd)
