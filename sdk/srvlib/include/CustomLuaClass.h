...
protected:
	...
	*/
	virtual void registClassName(lua_State *L, const char* className)
	{
		...
		lua_rawset(L, -3);
		//constructor
		setMetaNameClosure(L, "new", (void *)_new);
		//__gc
		setMetaClosure(L, TM_GC, (void *)_gc);
		//__index
		setMetaClosure(L, TM_INDEX, (void *)_get);
		//__newindex
		setMetaClosure(L, TM_NEWINDEX, (void *)_set);
		//__eq
		setMetaClosure(L, TM_EQ, (void *)_eq);
		//__add
		setMetaClosure(L, TM_ADD, (void *)_add);
		//__sub
		setMetaClosure(L, TM_SUB, (void *)_sub);
		//__mul
		setMetaClosure(L, TM_MUL, (void *)_mul);
		//__div
		setMetaClosure(L, TM_DIV, (void *)_div);
		//__mod
		setMetaClosure(L, TM_MOD, (void *)_mod);
		//__unm
		setMetaClosure(L, TM_UNM, (void *)_unm);
		//__len
		setMetaClosure(L, TM_LEN, (void *)_len);
		//__lt
		setMetaClosure(L, TM_LT, (void *)_lt);
		//__le
		setMetaClosure(L, TM_LE, (void *)_le);
		//__concat
		setMetaClosure(L, TM_CONCAT, (void *)_concat);
