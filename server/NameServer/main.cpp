...
//会话管理器版本号
#define NAME_MGR_KN_VERION   MAKEFOURCC(17, 3, 24, 1)
...
public:
	...
	{
		SetCurrentDirectory("../");
		CFileLogger flog(_T("NameServer_%s.log"), getCurrentTimeDesc());
...
//#ifndef _SERVICE
	//if (argc == 2 && _tcsncicmp("/cmd",argv[1],4)==0)//平时调试用
	{
		...
		{
			g_ConfigPath="NameServerLinux.txt";
		}
		ServerMain();
	}
//#else
...
VOID ServerMain()
{
	...
	CFileLogger flog(_T("./log/NameServer_%s.log"), getCurrentTimeDesc());
	CNameServer *pNameServer = new CNameServer();
