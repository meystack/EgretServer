...
namespace Util
{
    ...
    bool AStar::getPath(INT_PTR nEntityType, int fromX, int fromY, int targetX, int targetY, CVector<SMALLCPOINT> &outPaths, CScene* pScene)
    {
        if (!pScene) return false;
        //CrossCheckFuncType func = std::bind(&CScene::CanCross, pScene,std::placeholders::_1,std::placeholders::_2,std::placeholders::_3);
        CrossCheckFuncType func = [nEntityType,pScene](INT_PTR x,INT_PTR y){ return pScene->CanCross(nEntityType,x,y); };
        return getPath(fromX, fromY, targetX, targetY, outPaths, func);
    }

    bool AStar::getPath(int fromX, int fromY, int targetX, int targetY, CVector<SMALLCPOINT> &outPaths, CrossCheckFuncType& crossCheck)
    {
        int nLoopLimit = 1000;
        ...
        while (true)
        {
            ...
            for (i = 0; i < 8; i++)
            {
                ...
                if (pCell->MarkTag != this->m_nMarkTag || pCell->State == ASMapCell::CSNONE)
                {
                    ...
                    pCell->pNext = NULL;
                    if (pCell != this->m_LastOpenCell) pCell->pPre = nullptr;
                    if (!this->openCell(pCell)) return false;
                }
                else if (pCell->State == ASMapCell::CSOPEN)
                {
                    if (pCell->GCost > pCurCell->GCost + AStar::AS_MOVECOST[i & 1])
                    {
                        ...
                        pCell->btDir = i;
                        pCell->GCost = pCurCell->GCost + AStar::AS_MOVECOST[i & 1];
                        if (!this->reopenCell(pCell)) return false;
                    }
                }
    ...
    bool AStar::openCell(ASMapCell* pCell)
    {
        ...
        else
        {
            ...
            auto checkLoop = pLastCell;//死循环检查
            while (pLastCell->FValue < nFValue)
            {
                ...
                if (pLastCell->pPre->pNext != pLastCell) return false;
                pLastCell = pLastCell->pPre;
                if (checkLoop == pLastCell) return false;
            }
    ...
    bool AStar::reopenCell(ASMapCell* pCell)
    {
        int nFValue = pCell->HCost + pCell->GCost;
        pCell->FValue = nFValue;
        ...
        {
            auto checkLoop = pNextCell;//死循环检查
            do
            {
                pNextCell = pNextCell->pNext;
                if (checkLoop == pNextCell) return false;
            }
