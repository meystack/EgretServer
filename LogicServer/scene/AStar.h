...
namespace Util
{
    ...
    public:
         ...
         */
        bool getPath(INT_PTR nEntityType, int fromX, int fromY, int targetX, int targetY, CVector<SMALLCPOINT> &outPaths, CScene* pScene);
	     ...
	     * @param targetY 目的地坐标Y
         */
        bool getPath(int fromX, int fromY, int targetX, int targetY, CVector<SMALLCPOINT> &outPaths, CrossCheckFuncType& crossCheck);
         ...
         * @param pCell
         */
        bool openCell(ASMapCell* pCell);

        /**
         * 重新开启指定的格子更新移动估价值并重新再已开启格子链表中排序
         * @param pCell
         */
        bool reopenCell(ASMapCell* pCell);
